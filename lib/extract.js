...
var Extractor = (function () {
    ...
    Extractor.isValidStrategy = function (strategy) {
        return strategy === 'html' || strategy === 'js' || strategy==='json';
    };
    ...
    Extractor.prototype.extractJSON = function (filename, src) {
        var self = this;
        var syntax;
        var context;
        var singular;
        ...
        syntax = JSON.parse(src);

        var translations=syntax['fr'];

        for (var i = 0; i < Object.keys(translations).length; i++) {
            var key=Object.keys(translations)[i];
            singular=key;
            self.addString(reference, singular, plural, comments2String(extractedComments));
        };
    };
